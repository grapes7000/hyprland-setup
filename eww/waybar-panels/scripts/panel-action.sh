...
set -euo pipefail
case "${1:-}" in
  audio) case "${2:-}" in down) pamixer -d 5 2>/dev/null || true;; up) pamixer -i 5 2>/dev/null || true;; mute) pamixer -t 2>/dev/null || true;; mic-mute) pamixer --default-source -t 2>/dev/null || true;; previous|play-pause|next) playerctl "$2" 2>/dev/null || true;; pavucontrol) pavucontrol >/dev/null 2>&1 &;; esac ;;
